...
#3
landMosaic_cropland <- sort(c(hgRes$summary['landMosaic cropland / veg','estimate'],seq(-0.5,  0.5, len=199)))  
#4
landMosaic_vegetation <- sort(c(hgRes$summary['landMosaic vegetation / c','estimate'],seq(-1.2,  0.1, len=199))) 
#5
landbroadleaved_deciduous_forest <- sort(c(hgRes$summary['landbroadleaved deciduous','estimate'],seq(-0.9,  0.6, len=199)))
#6
landneedleleaved_evergreen_forest <- sort(c(hgRes$summary['landneedleleaved evergree','estimate'], seq(-0.7, 0.9, len=199))) 
#7
landOpen_needleleaved_deciduous_or_evergreen_forest <- sort(c(hgRes$summary['landOpen needleleaved dec','estimate'],seq(-1.7,  0, len=199)))  
#8
landmixed_broadleaved_and_needleleaved_forest <- sort(c(hgRes$summary['landmixed broadleaved and','estimate'],seq(-10.5e-01, -0.1e-01, len=199))) 
#9
landMosaic_forest_or_shrubland <- sort(c(hgRes$summary['landMosaic forest or shru','estimate'], seq(-0.98, 0.5, len=199))) 
#10
landMosaic_grassland <- sort(c(hgRes$summary['landMosaic grassland / fo','estimate'],seq(-0.7,  0.5, len=199))) 
#11
...
#14
landgrassland_or_woody_vegetation <- sort(c(hgRes$summary['landgrassland or woody ve','estimate'], seq(-1.9, 0.4, len=199))) 
#15
landArtificial_surfaces_and_associated_areas  <- sort(c(hgRes$summary['landArtificial surfaces a','estimate'], seq(-2.6, 1.6, len=199)))
#16
